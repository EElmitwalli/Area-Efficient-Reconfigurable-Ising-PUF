...
def main(rank=None, file=None, f_error=None, f_acc=None):
    with tf.Session() as sess:
        ...
        x = tf.placeholder(tf.float32, shape=[None, ising_size * ising_size * connection_size])

        # the correct answer y_
        y_ = tf.placeholder(tf.float32, [None, 1])

        # out = ttrl(x, rank, 1)
        out = ecp_trn(x, y1, rank, 1, ising_size, connection_size)
        ...
        train_step = tf.train.AdamOptimizer(l_rate).minimize(cross_entropy)  # 1e-2 eslam
...
def run(outfilepath, rank, iter):
    with open(outfilepath, "w+") as f, open("./out/log22_error_graph.txt", "w+") as f_error, open(
            "./out/log22_acc_graph.txt", "w+") as f_acc:
        for i in range(iter):
...
if __name__ == '__main__':
    run("./out/log22.txt", rank_val, 1)  # 1000 eslam
