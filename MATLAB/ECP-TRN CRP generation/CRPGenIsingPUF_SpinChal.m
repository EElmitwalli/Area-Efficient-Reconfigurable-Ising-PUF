...
isingSize=8;  %number of APUFs (x) in a side length (i.e. x*x-Ising-PUF)
...
parfor TrainingSet=1:CRPCount
    ...
    for steps=1:annealing_steps
        ...
        %update each bit according to chanllenge from neighbours
        for m=1:isingSize
            for n=1:isingSize
    ...
    StateBitsPhi = Transform(neighbourMatrix, isingSize^2, maxconn); %enable to store phi
    challengebitsPhi(TrainingSet,:)= reshape(StateBitsPhi.',1,[]);%Linear StateBitsNewAttackPhi
end
toc
